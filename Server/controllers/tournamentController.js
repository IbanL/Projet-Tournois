...
const getTournaments = async (req, res, next) => {
    try {
        const tournaments = await Tournament.find().select("-players -matches");
...
const getTournament = async (req, res, next) => {
    try {
        const tournament = await Tournament.findById(req.params.id).populate("players", ["_id", "name"]).populate("matches.rounds");;
        if (!tournament) {
...
const createRounds = async (tournament, next) => {
    ...
    for (let i = 0; i < tournament.nbRounds; i++) {
        ...
        tournament.matches.push({ rounds: round, roundNb });
    }
    tournament.save();
    } catch (error) {
        next(error);    
